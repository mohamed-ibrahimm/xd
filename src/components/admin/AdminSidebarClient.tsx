...
export default function AdminSidebarClient({ platformName, adminName }: Props) {
  ...
  const navSections = useMemo(() => [
    ...
    {
      title: 'المستخدمون والمالية',
      items: [
        {
          name: 'الطلاب والمستخدمون',
          href: '/admin/users',
          icon: Users,
        },
        {
    ...
    <div className="flex flex-col h-full justify-between">
      ...
      <div className="pt-4 border-t border-slate-200/80 dark:border-white/10 space-y-1 text-start">
        ...
        <button
          ...
          className="w-full flex items-center gap-2 px-3.5 py-2.5 rounded-2xl text-xs font-bold text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/30 transition-colors cursor-pointer"
        >
    ...
    <>
      ...
      <div className="lg:hidden w-full bg-white/95 dark:bg-[#0c0918]/95 border-b border-slate-200/80 dark:border-white/10 px-4 py-3 flex items-center justify-between sticky top-0 z-40 backdrop-blur-xl">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h2 className="text-xs font-black text-slate-900 dark:text-white leading-tight">{platformName}</h2>
            <span className="text-[10px] text-slate-500 dark:text-zinc-400">لوحة الإدارة</span>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[99999] lg:hidden animate-in fade-in duration-150">
          ...
          <div className="fixed inset-y-0 right-0 w-72 bg-white dark:bg-[#0c0918] p-5 shadow-2xl overflow-y-auto z-10 animate-in slide-in-from-right duration-200">
            <div className="flex items-center justify-between pb-3 mb-3 border-b border-slate-100 dark:border-white/10">
              <span className="text-xs font-black text-slate-900 dark:text-white">أقسام الإدارة</span>
              <button
                onClick={() => setMobileDrawerOpen(false)}
